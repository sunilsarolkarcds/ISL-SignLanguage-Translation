...
from keras.layers import Conv2D, ReLU,PReLU
...
def make_layers_keras(block, no_relu_layers,prelu_layers = []):
    layers = []
    for layer_name, v in block.items():
        if 'pool' in layer_name:
            # layer = nn.MaxPool2d(kernel_size=v[0], stride=v[1],
...
def make_layers_Mconv_keras(block,no_relu_layers):
    ...
    for layer_name, v in block.items():        
        if 'pool' in layer_name:
            # layer = nn.MaxPool2d(kernel_size=v[0], stride=v[1],
            #                         padding=v[2])
            layer=MaxPooling2D(pool_size=v[0], strides=v[1], padding='valid' if v[2] == 0 else 'same')
            layers.append((layer_name, layer))
        else:
            # conv2d = nn.Conv2d(in_channels=v[0], out_channels=v[1],
            #                    kernel_size=v[2], stride=v[3],
...
class bodypose_25_model(keras.Model):
    def __init__(
        ...
        **kwargs
        ...
        self.model0 = make_layers_keras(block0, no_relu_layers,prelu_layers)
        ...
        for k in blocks.keys():
            blocks[k] = make_layers_Mconv_keras(blocks[k], no_relu_layers)
        self.models = blocks
    ...
    def call(self,x):
        ...
        for s in range(4):
            ...
            outL2 = tout
            tout = Concatenate(axis=1)([out0, tout])#torch.cat([out0,tout],1)
        #L1 stage0
        ...
        outS0L1 = tout
        tout = Concatenate(axis=1)([out0, outS0L1, outL2])#torch.cat([out0,outS0L1,outL2],1)
        #L1 stage1
...
class bodypose_model(keras.Model):
    def __init__(
        ...
        **kwargs
        ...
        self.model0 = make_layers_keras(block0, no_relu_layers)
        ...
        for k in blocks.keys():
            blocks[k] = make_layers_keras(blocks[k], no_relu_layers)
    ...
    def call(self, x):
        ...
        out2 = Concatenate(axis=1)([out1_1, out1_2, out1])

        out2_1 = self.model2_1(out2)
        out2_2 = self.model2_2(out2)
        out3 = Concatenate(axis=1)([out2_1, out2_2, out1])

        out3_1 = self.model3_1(out3)
        out3_2 = self.model3_2(out3)
        out4 = Concatenate(axis=1)([out3_1, out3_2, out1])#torch.cat([out3_1, out3_2, out1], 1)

        out4_1 = self.model4_1(out4)
        out4_2 = self.model4_2(out4)
        out5 = Concatenate(axis=1)([out4_1, out4_2, out1])#torch.cat([out4_1, out4_2, out1], 1)

        out5_1 = self.model5_1(out5)
        out5_2 = self.model5_2(out5)
        out6 = Concatenate(axis=1)([out5_1, out5_2, out1])#torch.cat([out5_1, out5_2, out1], 1)
...
class handpose_model(keras.Model):
    ...
    def call(self, x):
        ...
        concat_stage2 = Concatenate(axis=1)([out1_1, out1_0])#torch.cat([out1_1, out1_0], 1)
        out_stage2 = self.model2(concat_stage2)
        concat_stage3 = Concatenate(axis=1)([out_stage2, out1_0])#torch.cat([out_stage2, out1_0], 1)
        out_stage3 = self.model3(concat_stage3)
        concat_stage4 = Concatenate(axis=1)([out_stage3, out1_0])#torch.cat([out_stage3, out1_0], 1)
        out_stage4 = self.model4(concat_stage4)
        concat_stage5 = Concatenate(axis=1)([out_stage4, out1_0])#torch.cat([out_stage4, out1_0], 1)
        out_stage5 = self.model5(concat_stage5)
        concat_stage6 = Concatenate(axis=1)([out_stage5, out1_0])#torch.cat([out_stage5, out1_0], 1)
